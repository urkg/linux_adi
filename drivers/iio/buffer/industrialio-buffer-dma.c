...
static void _iio_dma_buffer_block_done(struct iio_dma_buffer_block *block)
{
	if (block->state != IIO_BLOCK_STATE_DEAD) {
		...
		block->state = IIO_BLOCK_STATE_DONE;
		/*
		 * Out of tree: the outgoing queue is only needed for the legacy mmap interface.
		 * With the new dmabuf interface we have fences for signaling.
		 */
...
static bool iio_dma_buffer_can_use_fileio(struct iio_dma_buffer_queue *queue)
{
	 ...
	 *
	 * Out of tree: use num_blocks instead...
	 */
	return queue->fileio.enabled || !queue->num_blocks;
}
...
/* Out of tree: Legacy mmap interface that should be removed once dma-buf is upstream */
	...
	struct iio_buffer_block *block)
{
	...
	*block = queue->blocks[block->id]->block;
	/*
	 * We want to keep the rest of the code as close as possible with upstream. And since
	 * upstream does not have a struct iio_buffer_block embedded in struct iio_dma_buffer_block,
	 * the rest of the code does not update it's .bytes_used and .size members. Hence copy the
	 * right values  in here.
	 */
	block->bytes_used = queue->blocks[block->id]->bytes_used;
	block->size = queue->blocks[block->id]->size;
	...
	struct iio_buffer_block *block)
{
	...
	dma_block->bytes_used = block->bytes_used;
	...
	struct iio_buffer_block *block)
{
	...
	spin_lock_irq(&queue->list_lock);
	dma_block = list_first_entry_or_null(&queue->outgoing, struct iio_dma_buffer_block, head);
	if (!dma_block) {
		ret = -EAGAIN;
		spin_unlock_irq(&queue->list_lock);
		goto out_unlock;
	}
	list_del(&dma_block->head);
	spin_unlock_irq(&queue->list_lock);

	dma_block->state = IIO_BLOCK_STATE_DEQUEUED;

	*block = dma_block->block;
	block->size = dma_block->size;
	block->bytes_used = dma_block->bytes_used;
