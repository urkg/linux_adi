...
static int ad9739a_init(struct device *dev, const struct ad9739a_state *st)
{
	...
	if (ret)
		return ret;
	/* out of tree: in_range() still not available */
	if (fsc < AD9739A_FSC_MIN || fsc > AD9739A_FSC_MAX)
...
static int ad9739a_probe(struct spi_device *spi)
{
	...
	/* out of tree: in_range() still not available */
	if (st->sample_rate < AD9739A_MIN_DAC_CLK || st->sample_rate > AD9739A_MAX_DAC_CLK)
		return dev_err_probe(dev, -EINVAL,
