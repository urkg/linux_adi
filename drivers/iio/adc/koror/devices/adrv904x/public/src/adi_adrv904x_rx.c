...
#ifdef __KERNEL__
// ToDo - replce with linux/int_log.h after updating
        ...
        for (chanIdx = 0U; chanIdx < ADI_ADRV904X_MAX_RX_ONLY; ++chanIdx)
        {
            ...
            if (ADRV904X_BF_EQUAL(rxGain[configIndex].rxChannelMask, chanSel))
            {
                ...
                if (firstWrite == 0U)
                {
                    ...
                    halError = adrv904x_hal_SpiWrite(device->common.devHalInfo, dataPageSetup, DATA_TRANSACTION_1_SIZE);
                ...
                else
                {
                    ...
                    halError = adrv904x_hal_SpiWrite(device->common.devHalInfo, data, DATA_TRANSACTION_2_SIZE);
                    if (halError != ADI_HAL_ERR_OK)
                    {
                        recoveryAction = (adi_adrv904x_ErrAction_e) adi_common_hal_ErrCodeConvert(halError);
                        ADI_PARAM_ERROR_REPORT(&device->common,
