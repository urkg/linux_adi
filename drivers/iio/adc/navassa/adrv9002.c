...
static int adrv9002_rx_validate_profile(struct adrv9002_rf_phy *phy, unsigned int idx)
{
	...
	if (idx && rx_cfg[idx].profile.rxOutputRate_Hz != rx1->channel.rate) {
		dev_err(dev, "In rx2tx2, RX%d rate=%u must be equal to RX1, rate=%ld\n",
			idx + 1, rx_cfg[idx].profile.rxOutputRate_Hz, rx1->channel.rate);
		return -EINVAL;
	}

	if (idx && !rx1->channel.enabled) {
	...
	if (phy->rx2tx2) {
		...
		if (tx->rx_ref_clk && tx_cfg[idx].txInputRate_Hz != rx1->channel.rate) {
			...
			dev_err(dev, "In rx2tx2, TX%d rate=%u must be equal to RX1, rate=%ld\n",
				idx + 1, tx_cfg[idx].txInputRate_Hz, rx1->channel.rate);
				return -EINVAL;
		}
...
static int adrv9002_validate_profile(struct adrv9002_rf_phy *phy)
{
	...
	for (i = 0; i < ADRV9002_CHANN_MAX; i++) {
		...
		ret = adrv9002_rx_validate_profile(phy, i);
		if (ret)
			return ret;
	...
	for (i = 0; i < phy->chip->n_tx; i++) {
		...
		ret = adrv9002_tx_validate_profile(phy, i);
		if (ret)
			return ret;
